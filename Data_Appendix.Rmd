...
```
The minimum weight is 101 pounds and the maximum is 298. This makes sense because we filtered out the bottom coded (100 pounds or less) and top-coded weights (299 pounds or more) because these extreme values are not necessary to our analysis, and they represent categories of weights so it may skew our analysis if we included them as numberic values. There are no missing values, which is ideal. 

```{r}
favstats(~HRSLEEP, data = data)
```
The minimum value for hours of sleep is 1, and the maximum is 20. These both seem like very extreme values, but Q1 is 6 and Q3 is 8, which indicates that most individuals get between 6 and 8 hours of sleep, which makes sense. There are no missing values, which is ideal.

```{r}
favstats(~AGE, data = data)
```
The minimum age is 18 years and the maximum is 85 years. This makes sense because we filtered out all ages under 18 years because our analysis focuses on adults, and the age 85 represents all adults 85 years or older based on the IPUMS coding. There are no missing values, which is ideal.

```{r}
favstats(~BMI, data = data)
```

The minimum BMI is 15.96 and the maximum is 53.3. Both of these values makes sense, and there are no missing values. 
...
data <- data %>% 
  mutate(family_income = ifelse(INCFAM07ON == "10" |  INCFAM07ON == "11", "low_income", ifelse(INCFAM07ON == "12" | INCFAM07ON == "21" | INCFAM07ON == "22", "middle_income", "high_income"))) %>% 
  mutate(health_binary = ifelse(HEALTH == "1" | HEALTH == "2" | HEALTH == "3", "good", "poor")) %>% 
  select(WEIGHT, HRSLEEP, AGE, BMI, activity_score, sex, RACEA, 
         family_income, health_binary, WORFREQ) %>% 
  mutate(RACEA = droplevels(as_factor(RACEA)),
         WORFREQ = droplevels(as_factor(WORFREQ)))
```
...
After some initial data wrangling, our data set has 10 variables and 19925 observational units. The variables are:
...
6. sex is a character that indicates whether an individual's sex is Male or Female
7. RACEA is an integer that gives a code that corresponds to an individual's race category
8. family_income is a character that gives the income group of an individual(high, middle, or low).
9. health_binary is a character that indicates whether and individual's health is "good" or "poor"
10. WORFREQ is an integer that corresponds to a category (1-5) for how often an individual worries, 1 being daily and 5 being never.
...
# Have questions here -- include HRSLEEP*WORFREQ?
m1 <- lm(WEIGHT ~ HRSLEEP + WORFREQ + HRSLEEP*WORFREQ + RACEA + HRSLEEP*RACEA + AGE + BMI + activity_score + sex + family_income + health_binary, data = data)
...
```{r}
data <- data %>% 
  mutate(WORFREQ = ifelse(WORFREQ == "1", "1", "0"))
  # mutate(RACEA = ifelse(RACEA == ))
```
