...
In order to analyze our primary hypothesis based on our nested model $$\begin{split}
log(BMI) = \beta_0 + \beta_1 \cdot HRSLEEP +\beta'X \end{split}$$, we performed a t-test for the HRSLEEP coefficient with the hypotheses
...
$H_a: \beta_1 \ne 0$
  ...
  Our secondary hypothesis based on our full model $$\begin{split}
log(BMI) = \beta_0 + \beta_1 \cdot HRSLEEP + \beta_2 \cdot worfreq_binary + \\
 \beta_3 \cdot (HRSLEEP \cdot worfreq_binary) +\beta'X
\end{split}$$ was examined using three t-tests on the coefficients for HRSLEEP, worfreq_binary and the interaction HRSLEEP⋅worfreq_binary, using the following hypotheses, for i = 1, 2, and 3:
